...
TEST(matrix_test, SetCols1) {
  ...
  try {
    ...
    ASSERT_EQ(err.what(), ERROR_TEXT[1]);
  }
}

TEST(matrix_test, SetCols_and_GetColumns) {
  S21Matrix mtrx1(3, 3);
  mtrx1.SetColumns(1);
  ASSERT_EQ(mtrx1.GetColumns(), 1);
}
...
TEST(matrix_test, inverse2) {
  ...
  try {
    ...
    ASSERT_EQ(err.what(), ERROR_TEXT[2]);
  }
}
